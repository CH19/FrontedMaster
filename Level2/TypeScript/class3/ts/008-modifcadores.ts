...
abstract class OperacionBancaria{
    precioProducto: number;

    constructor(precioProudcto: number){
        this.precioProducto = precioProudcto;
    }
    operacion(saldo: number): number{
        return saldo - this.precioProducto;
    }
}
class Compra extends OperacionBancaria{
    saldoCliente: number = 0;
    constructor(saldoCliente: number, precioProducto: number){
...
class Persona{
    name: string = '';
    cedula: string = '';
    constructor(name: string, cedula: string){
        this.name = name;
        this.cedula = cedula;
    }
    decirNombre(){
...
class ClienteBancario extends Persona{
     public nombreBanco: string = 'Banco de Venezuela';
     private _saldo: number = 0;
     private _claveSuiche: string = '0';

     constructor(name: string, cedula: string, claveSuiche?: string, saldo?: number) {
        super(name, cedula)
        if(claveSuiche) this._claveSuiche = claveSuiche
        if(saldo) this._saldo = saldo;
    ...
    set claveSuiche(newClaveSuiche: string){
        this._claveSuiche = newClaveSuiche;
    }
    get claveSuiche(): string{
